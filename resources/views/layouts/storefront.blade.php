...
<body class="font-sans antialiased text-stone-800 bg-[#FCFBF7] min-h-screen flex flex-col selection:bg-amber-500 selection:text-white" x-data>
    <a href="#main-content" class="sr-only focus:not-sr-only focus:fixed focus:left-4 focus:top-4 focus:z-[70] focus:rounded-lg focus:bg-amber-500 focus:px-4 focus:py-3 focus:text-sm focus:font-bold focus:text-stone-950 focus:shadow-lg">
        Skip to content
    </a>
    ...
    <div class="bg-gradient-to-r from-[#0D4A2B] via-[#125834] to-[#0D4A2B] text-amber-100 text-xs py-2 px-4 shadow-sm">
        <div class="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-2 text-center sm:text-left">
            <div class="flex items-center gap-2 font-medium">
                ...
                <span class="hidden sm:inline">Express courier shipping to the <strong>UK, USA, Canada & Europe</strong> • Fresh local delivery daily</span>
                <span class="sm:hidden">Fresh local delivery • Diaspora courier available</span>
            </div>
    ...
    <header
        class="sticky top-0 z-40 bg-white/95 backdrop-blur border-b border-stone-200/80 transition-shadow duration-200"
        x-data="{ mobileMenuOpen: false, scrolled: false }"
        x-init="window.addEventListener('scroll', () => { scrolled = window.scrollY > 8 })"
        :class="scrolled ? 'shadow-lg' : 'shadow-xs'"
    >
            ...
            <div class="flex items-center justify-between h-20 gap-4">
                ...
                <div class="flex items-center gap-2 sm:gap-3 shrink-0">
                    ...
                    @else
                        ...
                        <a href="{{ route('register') }}" class="hidden sm:inline-flex items-center text-xs font-bold text-[#0D4A2B] border border-[#0D4A2B] hover:bg-[#0D4A2B] hover:text-white px-3 py-2 rounded-lg transition-colors whitespace-nowrap">
                            Register
                        </a>
                    @endauth
        ...
        >
            <a href="{{ route('home') }}" @click="mobileMenuOpen = false" class="block px-3 py-2 rounded-lg text-sm font-semibold {{ request()->routeIs('home') ? 'bg-emerald-50 text-[#0D4A2B]' : 'text-stone-700 hover:bg-stone-50' }}">
                Home
            </a>
            <a href="{{ route('menu.index') }}" @click="mobileMenuOpen = false" class="block px-3 py-2 rounded-lg text-sm font-semibold {{ request()->routeIs('menu.*') ? 'bg-emerald-50 text-[#0D4A2B]' : 'text-stone-700 hover:bg-stone-50' }}">
                Menu
            </a>
            <a href="{{ route('combos.index') }}" @click="mobileMenuOpen = false" class="block px-3 py-2 rounded-lg text-sm font-semibold {{ request()->routeIs('combos.*') ? 'bg-emerald-50 text-[#0D4A2B]' : 'text-stone-700 hover:bg-stone-50' }}">
                Combos & Deals
            </a>
            <a href="{{ route('catering.index') }}" @click="mobileMenuOpen = false" class="block px-3 py-2 rounded-lg text-sm font-semibold {{ request()->routeIs('catering.*') ? 'bg-emerald-50 text-[#0D4A2B]' : 'text-stone-700 hover:bg-stone-50' }}">
                Event Catering
            </a>
            <a href="{{ route('special-requests.create') }}" @click="mobileMenuOpen = false" class="block px-3 py-2 rounded-lg text-sm font-semibold {{ request()->routeIs('special-requests.*') ? 'bg-emerald-50 text-[#0D4A2B]' : 'text-stone-700 hover:bg-stone-50' }}">
                Special Requests
            </a>
            <a href="{{ route('reviews.index') }}" @click="mobileMenuOpen = false" class="block px-3 py-2 rounded-lg text-sm font-semibold {{ request()->routeIs('reviews.*') ? 'bg-emerald-50 text-[#0D4A2B]' : 'text-stone-700 hover:bg-stone-50' }}">
                Customer Reviews
            </a>
            <a href="{{ route('about') }}" @click="mobileMenuOpen = false" class="block px-3 py-2 rounded-lg text-sm font-semibold {{ request()->routeIs('about') ? 'bg-emerald-50 text-[#0D4A2B]' : 'text-stone-700 hover:bg-stone-50' }}">
                Our Story
            </a>
            <a href="{{ route('contact') }}" @click="mobileMenuOpen = false" class="block px-3 py-2 rounded-lg text-sm font-semibold {{ request()->routeIs('contact') ? 'bg-emerald-50 text-[#0D4A2B]' : 'text-stone-700 hover:bg-stone-50' }}">
                Contact Us
            </a>
    ...
    <div class="fixed bottom-6 right-6 z-30">
        ...
        >
            ...
            <span class="font-bold text-sm tracking-wide hidden sm:inline">WhatsApp Order</span>
        </a>
    </div>
            ...
            <div class="pt-8 flex flex-col sm:flex-row items-center justify-between gap-6 text-xs text-stone-500">
                <div class="flex flex-col sm:flex-row items-center gap-4">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <a href="#" aria-label="Facebook" class="w-7 h-7 rounded-full bg-stone-800 hover:bg-amber-500 text-stone-400 hover:text-stone-950 flex items-center justify-center transition-colors">
                        ...
                        <a href="#" aria-label="Twitter / X" class="w-7 h-7 rounded-full bg-stone-800 hover:bg-amber-500 text-stone-400 hover:text-stone-950 flex items-center justify-center transition-colors">
                ...
                <div class="flex items-center gap-2 flex-wrap justify-center">
                    <span class="inline-flex items-center gap-1 bg-stone-800 border border-stone-700 text-stone-300 text-[10px] font-bold px-2.5 py-1 rounded-full">
                        💳 Paystack <span class="text-amber-400">(NGN)</span>
                    </span>
                    <span class="inline-flex items-center gap-1 bg-stone-800 border border-stone-700 text-stone-300 text-[10px] font-bold px-2.5 py-1 rounded-full">
                        💳 Stripe <span class="text-amber-400">(GBP · USD · EUR)</span>
                    </span>
                    <span class="inline-flex items-center gap-1 bg-stone-800 border border-stone-700 text-stone-300 text-[10px] font-bold px-2.5 py-1 rounded-full">
                        🏦 Bank Transfer
                    </span>
